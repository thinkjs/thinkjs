...
module.exports = think.Class({
  ...
  loadMVC: function(){
    ...
    list.forEach(function(type){
      ...
      if (think.mini) {
        think.alias(think.config('default_module') + '/' + type, think.getModulePath() + '/' + think.dirname[type], true);
      }else{
  ...
  loadTemplate: function(){
    ...
    think.cache(think.cache.template, data);
  },
  /**
   * load error message
   * @return {} []
  ...
  loadErrorMessage: function(){
    var config = require(think.THINK_LIB_PATH + '/config/error.js');
    var filepath = think.APP_PATH + '/';
    if (think.mini) {
      filepath += think.dirname.config + '/error.js';
    }else{
      filepath += think.dirname.common + '/' + think.dirname.config + '/error.js';
    }
    var appConfig = think.safeRequire(filepath);
    think.ERROR_MESSAGE = think.extend({}, config, appConfig);
  },
  /**
