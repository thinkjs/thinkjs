...
var App = module.exports = think.Class({
  ...
  error: function(err){
    ...
    http.res.statusCode = think.config('error.code') || 500;
    http.setHeader('Content-Type', 'text/html; charset=' + think.config('encoding'));
    if (think.debug) {
      http.res.end(message);
    }else{
      var readStream = fs.createReadStream(think.config('error.tpl'));
  ...
  dispatcher: function(){
    ...
    return this.hook('resource_check').then(function(){
      ...
      self.http.config = think.getModuleConfig(self.http.module);
    })
  },
  /**
   * exec logic
...
App.http = function(){
  ...
  if (cluster.isMaster) {
    console.log('use cluster, fork nums ' + clusterNums);
