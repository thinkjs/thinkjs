...
var App = module.exports = think.Class({
  ...
  error: function(err){
    ...
    if (callback) {
      ...
      if (!think.isFunction(callback)) {
        throw new Error(think.message('CONFIG_NOT_FUNCTION', 'error.callback'));
      }
      callback(err, http);
    }else{
   ...
   */
  execLogic: function(){
    var module = think.mini ? think.config('default_module') : this.http.module;
    var name = module + '/' + think.dirname.logic + '/' + this.http.controller;
    var cls = think._aliasExport[name];
    if (!cls) {
  ...
  execController: function(){
    var http = this.http;
    var name = think.dirname.controller + '/' + http.controller;
    if (!think.mini) {
...
App.createServer = function(){
  ...
  if (handle) {
    ...
    if (!think.isFunction(handle)) {
      throw new Error(think.message('CONFIG_NOT_FUNCTION', 'create_server'));
    }
