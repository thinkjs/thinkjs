...
module.exports = Class(function() {
  ...
  return {
    ...
    get: function(name) {
      if (name === undefined) {
        var result = {};
        for(var key in this.http.get) {
          result[key] = escapeComparison(this.http.get[key] || '');
        }
        return result;
      }
      return escapeComparison(this.http.get[name] || '');
    },
    /**
     * 获取POST参数
     * @param  {[type]} name [description]
    ...
    post: function(name) {
      if (name === undefined) {
        var result = {};
        for(var key in this.http.post) {
          result[key] = escapeComparison(this.http.post[key] || '');
        }
        return result;
      }
      return escapeComparison(this.http.post[name] || '');
    },
    /**
     * 获取参数
     * @param  {[type]} name [description]
