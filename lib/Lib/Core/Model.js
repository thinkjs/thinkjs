...
var Model = module.exports = Class(function(){
    ...
    return {
        ...
        initDb: function(){
            ...
            var md5Value = md5(JSON.stringify(config));
            if (!dbInstances[md5Value]) {
                if (config && isString(config) && config.indexOf('/') === -1) {
                    config = C(config);
                }
                dbInstances[md5Value] = thinkRequire("Db").getInstance(config);
            }
            this.db = dbInstances[md5Value];
            return this.db;
        },
        /**
         * 获取模型名
            ...
            return promise.then(function(fields){
                ...
                if (isObject(options.where) && !isEmpty(fields)) {
                    ...
                    for(var key in options.where){
                        ...
                        }else if(key.substr(0, 1) !== "_" && !keyReg.test(key)){
 ...
 * @return {[type]} [description]
 */
Model.close = global.closeDb = function(){
    "use strict";
