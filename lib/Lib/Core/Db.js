...
var Db = module.exports = Class(function(){
  ...
  return {
    ...
    initConnect: function(master){
      var hosts = getDbItem(this.config.hostname);
      //单个host
      if (hosts.length === 1) {
    ...
    multiConnect: function(master){
      ...
      var index = 0;
      var hostnameLength = getDbItem(config.hostname).length;
      //读写分离
      ...
      var dbConfig = {
        type: config.type,
        username: getDbItem(config.username, index),
        password: getDbItem(config.password, index),
        hostname: getDbItem(config.hostname, index),
...
Db.parseConfig = function(config){
  ...
  return {
    ...
    socket: config.db_socket || C('db_socket'),
