  ...
  return {
    ...
    _request: function(){
      var req = {
        ...
        //请求用户的ip
        ip: function(forward){
          var proxy = C('use_proxy') || this.host === this.hostname;
          var ip;
          if (proxy) {
            if (forward) {
      ...
      this.http.pathname = path.normalize(urlInfo.pathname || '').replace(/\\/g, '/');
      var query = extend({}, this.req.params, urlInfo.query);
      //query只记录?后面的参数
      this.http.query = this.filterQuery(query);
      //get包含路由解析追加的参数
      this.http.get = extend({}, query);
      //主机名，带端口
