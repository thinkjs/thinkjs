    ...
    return {
        ...
        add: function(data, options, replace){
            ...
            return this.parseOptions(options).then(function(options){
                ...
                data[self.getPk()] = self.db.getLastInsertId();
                return getPromise(self._afterAdd(data, parsedOptions));
            });
        },
        /**
         * 插入多条数据
        ...
        delete: function(options){
            ...
            return this.parseOptions(options).then(function(options){
                ...
                return self._afterDelete(data, parsedOptions);
            });
        },
        /**
         * 更新前置操作
        ...
        update: function(data, options){
            ...
            return this.parseOptions(options).then(function(options){
                ...
                if (isEmpty(self._options.where) && isEmpty(parsedOptions.where)) {
                    // 如果存在主键数据 则自动作为更新条件
                    if (!isEmpty(data[pk])) {
                        var where = {};
                        where[pk] = data[pk];
                        parsedOptions.where = where;
                        pkValue = data[pk];
                        delete data[pk];
                    }else{
                ...
                return self._afterUpdate(data, parsedOptions);
            }).then(function(data){
