 ...
 */
var Memcache = inherits(EventEmitter, function(){
	return {
		 ...
		 * @return {[type]}            [description]
		 */
		increment: function(key, step, callback){
			if (typeof step === 'function') {
				callback = step;
				step = 1;
			}
			step = step || 1;
			return this.query('incr ' + key + ' ' + step, 'simple', callback);
		},
		/**
		 * 减少
		 * @param  {[type]}   key      [description]
		 ...
		 * @return {[type]}            [description]
		 */
		decrement: function(key, step, callback){
			if (typeof step === 'function') {
				callback = step;
				step = 1;
			}
			step = step || 1;
			return this.query('decr ' + key + ' ' + step, 'simple', callback);
		},
		/**
		 * 关闭
		 * @return {[type]} [description]
...
module.exports = Class(function(){
	return {
		...
		connect: function(){
			if (!this.handle) {
				this.handle = Memcache(this.port, this.hostname);
			}
		},
		get: function(key){
			var deferred = getDefer();
			this.handle.get(key, function(error, value){
				if (error) {
		...
		delete: function(key){
			var deferred = getDefer();
			this.handle.delete(key, function(error){
				if (error) {
					deferred.reject(error);
				}else{
					deferred.resolve();
	...
	var result = {};
	["increment", "decrement"].forEach(function(item){
		result[item] = function(name, step){
			var deferred = getDefer();
			this.handle[item](name, step, function(error){
				if (error) {
					deferred.reject(error);
				}else{
					deferred.resolve();
	...
	result.inc = result.increment;
	result.dec = result.decrement;

	['set', 'add', 'replace', 'append', 'prepend'].forEach(function(item){
		result[item] = function(name, value, lifetime){
			var deferred = getDefer();
			this.handle[item](name, value, function(error){
				if (error) {
					deferred.reject(error);
				}else{
					deferred.resolve();
