 ...
 */
global.think_require_path = function(name){
    if (_alias[name]) {
        return _alias[name];
    };
    var result = '';
...
global.think_require = function(name){
    var path = think_require_path(name);
    if (path) {
...
global.tag = function(name, http, data){
    ...
    if (tags.length) {
        ...
        return get_promise(runBehavior());
    };
    return get_promise(http.tag_data);
}
/**
 * 配置读取和写入
 */
...
global.A = function(name, http, data){
    ...
    var path = think_require_path(gm + "Controller");
    if (path) {
...
global.M = global.D = function(name, tablePrefix, connection){
    ...
    name = name.split(":");
    var path = think_require_path(name[0] + "Model");
    if (path) {
 ...
 * @return {[type]}            [description]
 */
global.send_status = function(res, statusCode){
    res.statusCode = statusCode || 403;
    res.end();
}