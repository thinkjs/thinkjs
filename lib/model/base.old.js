...
var Db = think.adapter('db', 'base');
var Filter = think.require('filter');
var Valid = think.require('valid');
 ...
 * @type {[type]}
 */
var Model = module.exports = think.Class({
  // 当前数据库操作对象
  db: null,
  // 主键名称
  pk: 'id',
  ...
  _validData: function(data){
    ...
    var json_message = JSON.stringify(result);
    var err = new Error(json_message.slice(1, -1));
  ...
  selectAdd: function(options){
    ...
    return Promise.all([this.parseOptions(), promise]).then(function(data){
      var fields = data[0].field || self._fields._field;
