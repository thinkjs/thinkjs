...
module.exports = class Application {
  ...
  _watcherCallBack(fileInfo){
    ...
    //reload all workers
    if(this.gracefulInstance){
      this.gracefulInstance.forceReloadWorkers();
    }
  }
  /**
   * start watcher
  ...
  runInMaster(){
    ...
    instance.master();
    this.gracefulInstance = instance;
  }
  /**
   * run in worker
   * @param {Object} argv 
  ...
  runInWorker(argv){
    ...
    think.app.once('startServer', () => {
      if(instance.isFirstWorker()){
        ...
        if(process.env.THINK_WORKERS){
          think.logger.info(`Cluster workers: ${process.env.THINK_WORKERS}`);
        }
      }
