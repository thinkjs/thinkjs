...
const appendData = helper.promisify(fs.appendFile, fs);
const isExists = helper.promisify(fs.exists, fs);
const fileStat = helper.promisify(fs.stat, fs);
...
class FileStore {
    ...
    append(relativePath, data) {
        ...
        return appendData(filePath, data).catch(err => {
            return Promise.reject(err);
        });
    }
    ...
    has(relativePath) {
        const filePath = this[getFilePath](relativePath);
        return isExists(filePath).catch(err => {
            return Promise.reject(err);
        });
    }
    ...
    info(relativePath) {
        ...
        return fileStat(filePath).catch(err => {
            return Promise.reject(err);
        })
    }
}
