...
program
  .usage('<middleware-name> [module-name]');
...
program.on('--help', function() {
  ...
  console.log();
  console.log(chalk.gray('    # create a middleware with the name ' + chalk.gray.underline.bold('user')));
  console.log('    $ thinkjs middleware user');
  console.log();
  console.log(chalk.gray('    # create a middleware with the name ' + chalk.gray.underline.bold('user') + ' in multi-module mode'));
  console.log('    $ thinkjs middleware user home');
  console.log();
});
...
const run = new Run({
  ...
  targetPath: appPath,
  options: { name: thinkjsInfo.metadata.name, command: 'middleware', maps: 'middleware', context },
  done(err, files) {
