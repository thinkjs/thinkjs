...
test.serial('set key1 -> get ke1 -> set key2 ->get key2 ->delCache ke1 key2', async t => {
  ...
  const regRes1 = await redisCache.deleteRegKeys(regKey1);
  const regRes = await redisCache.deleteRegKeys(regKey);
  const res1 = await redisCache.get(key1);
  const res2 = await redisCache.get(key2);
  t.true(g1.a.b === content1.a.b && g2.c.d === content2.c.d && regRes1 === 'NO_KEYS' && regRes === 'OK' && res1 === void 0 && res2 === void 0);
});
